...
// export const getAllProductSearchCategory = async (data) => {

//     try {
//         const res = await axiosJWT.get(`/api/product/get-all-product-search`, data)
...
export const getAllProductSearchCategory = async (data) => {
    try {
        // Nếu data là chuỗi, chuyển nó thành đối tượng phù hợp cho API
        const payload = typeof data === "string" ? { type: data } : data;

        const res = await axiosJWT.get(`/api/product/get-all-product-search`, {
            params: payload,
        });
